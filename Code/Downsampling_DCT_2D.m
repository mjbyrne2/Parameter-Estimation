...
F_c = dct2(F);    % DFT of image F
K_c = dct2(fftshift(K));    % DFT of kernel K
G_c = K_c.*F_c;
G = idct2(G_c);   % Convolved image G
...
for j = 1:R    
    ...
    G_noise_c = dct2(squeeze(G_noise(:,:,j)));    % DCT of jth data set g_noise
    ...
    for i = 0:M-1  
        ...
        if i == 0
            best_lambda(j) = optimalParameter2D(G_noise_c,K_c,...
                ones(N,N),F_c);
            best_regf(:,:,j) = ifft2(conj(G_noise_c).*K_c./...
                ((abs(K_c).^2) + (best_lambda(j)*ones(N,N))));
        end
        
        [upre_vectors(i+1,:,j),upre_lambda(j,i+1)] = UPREparameter(gn_c,...
            kn_c,ones(n^2,1),eta,L,r);
        upre_lambda(j,i+1) = upre_lambda(j,i+1);  % Scale the lambda
        
        [gcv_vectors(i+1,:,j),gcv_lambda(j,i+1)] = GCVparameter(gn_c,...
            kn_c,ones(n^2,1),L,r);
...
%         mdp_lambda(j,i) = mdp_lambda(j,i);  % Scale the lambda
        ...
        upre_regf(i+1,:,:,j) = idct2(G_noise_c.*K_c./...
                ((K_c.^2) + (upre_lambda(j,i+1)*ones(N,N))));
        
        gcv_regf(i+1,:,:,j) = idct2((G_noise_c.*K_c./...
                ((K_c.^2) + (gcv_lambda(j,i+1)*ones(N,N)))));
