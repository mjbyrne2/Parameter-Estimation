...
If $p$ is a multiple of $n$, then $p = mn$ for some $m \in \mathbb{Z}$. Thus,
\[\exp(i\theta_{p\ell}) = \exp(i2\pi{mn}\ell/n) = \exp(i2\pi{m\ell}) = 1,\]
and so
...
\begin{align*}
f_U(u) &= \frac{1}{\sqrt{2\pi\noiseSD^2}}\left[\exp\left(\frac{-(g_1^{-1}(u))^2}{2\noiseSD^2}\right)\left|\frac{d}{du}g_1^{-1}(u)\right| + \exp\left(\frac{-(g_2^{-1}(u))^2}{2\noiseSD^2}\right)\left|\frac{d}{du}g_2^{-1}(u)\right|\right] \\
...
The statistics of the DFT of white noise are now established, which allows the focus to be shifted towards analyzing the combination $\bVec + \noiseVec = \dVec$. By the properties of the multivariate normal distribution, if $\noiseVec \sim \mathcal{N}(\zeroVec,\noiseSD^2 I)$ then $\dVec \sim \mathcal{N}(\bVec,\noiseSD^2 I)$ because $\bVec$ is a constant vector. Thus the distribution of the components of $\Re(\dft{\dVec})$ and $\Im(\dft{\dVec})$ are readily obtained by extending previous results.

\begin{lemma}[Extension of Lemma \ref{lem:Component distributions}]
\label{lem:Component distributions ext.}
Let $\mathbf{X} = \Re(\dft{\dVec})$ and $\mathbf{Y} = \Im(\dft{\dVec})$, where $\dVec$ is a random $n$-vector with $\dVec \sim \mathcal{N}(\bVec,\noiseSD^2 I)$. Also let $J = \{0,n/2\}$ if $n$ is even and $J = \{0\}$ if $n$ is odd. Then for $0 \leq j \leq n-1$, the distribution of $X_j$ and $Y_j$ is as follows.  
\[X_j \sim \begin{cases}
...
\begin{proof}
Let $X$ and $Y$ be as required. Using \eqref{eq:Components Re(F) and Im(F)}, $X_j$ can be written as
\[X_j = \sum_{k=0}^{n-1} \frac{1}{\sqrt{n}}\cos\left(\frac{-2\pi{jk}}{n}\right)\dVec_k.\]
Since the covariance matrix of $\dVec$ is $\noiseSD^2 I$, the components $\dVec_k$ are independent and distributed $\mathcal{N}(\bVec_k,\noiseSD^2)$ for all $0 \leq k \leq n-1$. Thus by the properties of a sum of independent normal random variables \cite[p.~184]{CasellaBerger02},
\begin{align*}
...
In contrast, however, to the results regarding the components of $|\dft{\noiseVec}|^2$, the distribution of the components of $|\dft{\dVec}|^2$ is somewhat  complicated. To illustrate this, consider $U = X_0^2$, where $\mathbf{X} = \Re(\dft{\dVec})$ for odd-length $\dVec$ distributed $\mathcal{N}(\bVec,\noiseSD^2 I)$. To simplify notation, let $\mathbf{\mu} = \Re(\dft{\bVec})$ so that by Lemma \ref{lem:Component distributions ext.}, $X_0 \sim \mathcal{N}(\mu_0,\noiseSD^2)$. Applying the same transformation technique from Theorem \ref{thm:Mag. squared theorem}, the probability density function of $U$ is 
\begin{align*}
...
Before the distribution of the scaled components of $\dft{\dVec}$ is stated, Lemma \ref{lem:App of Ind Thm 2} establishes the independence of the (squared) real and imaginary parts.

\begin{lemma}
\label{lem:App of Ind Thm 2}
Let $\dVec$ be a random $n$-vector with $\dVec \sim \mathcal{N}(\bVec,\noiseSD^2 I)$, $J = \{0,n/2\}$ if $n$ is even and $J = \{0\}$ if $n$ is odd. Define the diagonal matrix $M$ by
\[M_{j,j} = \begin{cases}
1/\noiseSD & j \in J \\
\sqrt{2}/\noiseSD & j \not\in J
\end{cases}.\]
Then $\Re((M\dft{\dVec})_j)^2$ and $\Im((M\dft{\dVec})_k)^2$ are independent for all $0 \leq j,k \leq n-1$.
\end{lemma}
\begin{proof}
Let $\dVec \sim \mathcal{N}(\bVec,\noiseSD^2 I)$, $\mathbf{X} = \Re(M\dft{\dVec})$, and $\mathbf{Y} = \Im(M\dft{\dVec})$. Since $\dft{\dVec} = \dft{\bVec} + \dft{\noiseVec}$, $X_j^2$ and $Y_k^2$ can be expressed as 
\[X_j^2 = [M_{j,j}(\Re(\dft{g}_j) + \Re(\dft{\noise}_j))]^2, \quad Y_k^2 = [M_{k,k}(\Im(\dft{g}_k) + \Im(\dft{\noise}_k))]^2.\]
$X_j^2$ and $Y_k^2$ are thus functions of only the random variables $\Re(\dft{\noise}_j)$ and $\Im(\dft{\noise}_k)$, respectively. $\Re(\dft{\noise}_j)$ and $\Im(\dft{\noise}_k)$ are independent by Lemma \ref{lem:App of Ind Thm}, and therefore $X_j^2 = \Re((M\dft{\dVec})_j)^2$ and $Y_k^2 = \Im((M\dft{\dVec})_k)^2$ are independent as well.
\end{proof}

The distribution of the scaled components of $\dft{\dVec}$ can now be stated. 
...
\sqrt{2}/\noiseSD & j \not\in J
\end{cases}.\]
Then for $0 \leq j \leq n-1$, the distribution of $|(M\dft{\dVec})_j|^2 = \Re((M\dft{\dVec})_j)^2 + \Im((M\dft{\dVec})_j)^2$ is as follows.
\[|(M\dft{\dVec})_j|^2 \sim \begin{cases}
...
\end{theorem}
\begin{proof}
Let $\dVec \sim \mathcal{N}(\bVec,\noiseSD^2 I)$, $\mathbf{X} = \Re(M\dft{\dVec})$, and $\mathbf{Y} = \Im(M\dft{\dVec})$. Again without loss of generality, we assume that $n$ is odd so that the two cases to be considered are $j = 0$ and $j \neq 0$. \par
First, the components of $\mathbf{X}^2$ will be determined. If $j = 0$, then $\Re(\dft{\dVec}_j) \sim \mathcal{N}(\mu_j,\noiseSD^2)$ from Lemma \ref{lem:Component distributions ext.}, where $\mathbf{\mu} = \Re(\dft{\bVec})$ for readability. Thus by the properties of the normal distribution \cite[p.~184]{CasellaBerger02}, $\Re(\dft{\dVec}_j/\noiseSD) = \Re((M\dft{\dVec})_j) = X_j \sim \mathcal{N}(\sqrt{2\lambda},1)$ where $\lambda = (\mu_j/\noiseSD)^2/2$. Applying the transformation technique used in Theorem \ref{thm:Mag. squared theorem}, the probability density function of $V = X_j^2$ is
...
\end{align*}
Hyperbolic cosine is an entire function with $\cosh(z) = \sum_{\ell=0}^{\infty} z^{2\ell}/(2\ell)!$ as its Taylor expansion.  From \cite[p.~255]{AS}, 
\begin{equation}
...
If $j \neq 0$, then Lemma \ref{lem:Component distributions ext.} gives that $\Re(\dft{\dVec}_j) \sim \mathcal{N}(\Re(\dft{g}_j),\noiseSD^2/2)$. This implies that $\Re((\sqrt{2}\dft{\dVec}/\noiseSD)_j) = \Re((M\dft{\dVec})_j) = X_j \sim \mathcal{N}(\sqrt{2\lambda},1)$, now with $\lambda = (\mu_j/\noiseSD)^2$. Thus the same argument can be applied so that $X_j^2 \sim \NCchi^2(1,(\mu_j/\noiseSD)^2) = \NCchi^2(1,(\Re(\dft{g}_j)/\noiseSD)^2)$. \par 
With the components of $\mathbf{X}^2$ examined, focus can directed towards the components of $\mathbf{Y}^2$. If $j = 0$, then Lemma \ref{lem:Component distributions ext.} gives that $\Im(\dft{\dVec}_j) = 0$ (a constant random variable). This implies that $\Im(\dft{\dVec}_j/\noiseSD) = \Im((M\dft{\dVec})_j) = Y_j = 0$, meaning $Y_j^2 = 0$ as well. If $j \neq 0$, then $\Im(\dft{\dVec}_j) \sim \mathcal{N}(\Im(\dft{g}_j),\noiseSD^2/2)$. Thus by the argument for $X_j$, $Y_j^2 \sim \NCchi^2(1,(\Im(\dft{g}_j)/\noiseSD)^2)$. \par 
...
From Theorem \ref{thm:Mag. squared theorem ext.}, there are conditions which simplify the distribution of $|(M\dft{\dVec})_j|^2$. If $j = 0$ (or $n/2$ when $n$ is even), the noncentrality parameter of the noncentral chi-squared distribution is $(\Re(\dft{g}_0))^2/2\noiseSD^2$. By the definition of the DFT,
...
Ultimately there is a trade-off between the frequency content of the function $g(t)$ and the variance in the added noise. If the function $g(t)$ has a small amount of high frequency content (relative to the variance of the noise), then the statistics of corresponding terms $|(M\dft{\dVec})_j|^2$ will resemble those of chi-squared random variables. \par 
...
\end{cases}.\]
Then 
\[\E\left(|(M\dft{\dVec})_j|^2\right) = \begin{cases}
\dfrac{1}{2}\left(2 + \dfrac{|\dft{g}_j|^2}{\noiseSD^2}\right), & j \in J \\
2 + \dfrac{|\dft{g}_j|^2}{\noiseSD^2}, & j \not\in J \end{cases}\]
and
\[\Var\left(|(M\dft{\dVec})_j|^2\right) = \begin{cases}
2\left(1 + \dfrac{|\dft{g}_j|^2}{\noiseSD^2}\right), & j \in J \\
4\left(1 + \dfrac{|\dft{g}_j|^2}{\noiseSD^2}\right), & j \not\in J \end{cases}.\]
\end{corollary}

The final statistical result presented here is the covariance of $|M_j\dft{\dVec}_j|^2$ and $|M_k\dft{\dVec}_k|^2$, which will be used in Chapter \ref{ch:Parameter estimation methods}.  

% Here is the univariate approach
A step in Theorem \ref{thm:Cov of mag squared} is to determine covariances between components of $M\dft{\dVec} = M\dft{\bVec} + M\dft{\noiseVec}$. Lemma \ref{lem:App of Ind Thm 2} establishes that the real and imaginary parts of $M\dft{\noiseVec}$ are independent, but Lemma \ref{lem:App of Ind Thm 3} establishes the relationship between any two real or imaginary components of $M\dft{\noiseVec}$.
...
The covariance of $|M_j\dft{\dVec}_j|^2$ and $|M_k\dft{\dVec}_k|^2$ for $0 \leq j < k \leq n-1$ can now be determined.
...
\[\Cov\left(|M_j\dft{\dVec}_j|^2,|M_k\dft{\dVec}_k|^2\right) = \begin{cases}
...
\begin{align*}
&\Cov\left(|M_j\dft{\dVec}_j|^2,|M_k\dft{\dVec}_k|^2\right) \\
&= \Cov\left(\Re(M_j\dft{\dVec}_j)^2 + \Im(M_j\dft{\dVec}_j)^2, \Re(M_k\dft{\dVec}_k)^2 + \Im(M_k\dft{\dVec}_k)^2\right) \\
&= \Cov\left(\Re(M_j\dft{\dVec}_j)^2 ,\Re(M_k\dft{\dVec}_k)^2\right) + \Cov\left(\Re(M_j\dft{\dVec}_j)^2 ,\Im(M_k\dft{\dVec}_k)^2\right) \\
&+ \Cov\left(\Im(M_j\dft{\dVec}_j)^2 ,\Re(M_k\dft{\dVec}_k)^2\right) + \Cov\left(\Im(M_j\dft{\dVec}_j)^2 ,\Im(M_k\dft{\dVec}_k)^2\right).
\end{align*}
By Lemma \ref{lem:App of Ind Thm 2}, $\Re(M_j\dft{\dVec}_j)^2$ and $\Im(M_k\dft{\dVec}_k)^2$ are independent for all $0 \leq j,k \leq n-1$, meaning that their covariance is zero. Thus, the covariance simplifies to
\begin{align*}
    \Cov\left(|M_j\dft{\dVec}_j|^2,|M_k\dft{\dVec}_k|^2\right) &= \Cov\left(\Re(M_j\dft{\dVec}_j)^2 ,\Re(M_k\dft{\dVec}_k)^2\right) \\
    &+ \Cov\left(\Im(M_j\dft{\dVec}_j)^2 ,\Im(M_k\dft{\dVec}_k)^2\right).
\end{align*}
A further simplification can be made by using $\dft{\dVec} = \dft{\bVec} + \dft{\noiseVec}$ and expanding the squared terms:
\begin{align*}
&\Cov\left(\Re(M_j\dft{\dVec}_j)^2 ,\Re(M_k\dft{\dVec}_k)^2\right) \\
&= \Cov\left([\Re(M_j\dft{g}_j) + \Re(M_j\dft{\noise}_j)]^2,[\Re(M_k\dft{g}_k) + \Re(M_k\dft{\noise}_k)]^2\right) \\
&= \Cov\left(\Re(M_j\dft{g}_j)^2,\Re(M_k\dft{g}_k)^2\right) + \Cov\left(\Re(M_j\dft{g}_j)^2, 2\Re(M_k\dft{g}_k)\Re(M_k\dft{\noise}_k)\right) \\
&+ \Cov\left(\Re(M_j\dft{g}_j)^2, \Re(M_k\dft{\noise}_k)^2\right) + \Cov\left(2\Re(M_j\dft{g}_j)\Re(M_j\dft{\noise}_j), \Re(M_k\dft{g}_k)^2 )\right) \\
&+ \Cov\left(2\Re(M_j\dft{g}_j)\Re(M_j\dft{\noise}_j), 2\Re(M_k\dft{g}_k)\Re(M_k\dft{\noise}_k)\right) \\
&+ \Cov\left(2\Re(M_j\dft{g}_j)\Re(M_j\dft{\noise}_j), \Re(M_k\dft{\noise}_k)^2 )\right) + \Cov\left(\Re(M_j\dft{\noise}_j)^2,\Re(M_k\dft{g}_k)^2\right) \\
&+ \Cov\left(\Re(M_j\dft{\noise}_j)^2, 2\Re(M_k\dft{g}_k)\Re(M_k\dft{\noise}_k)\right) + \Cov\left(\Re(M_j\dft{\noise}_j)^2,\Re(M_k\dft{\noise}_k)^2\right).
\end{align*}
...
\[\Cov\left(|M_j\dft{\dVec}_j|^2,|M_k\dft{\dVec}_k|^2\right) = \Var\left(|M_j\dft{\dVec}_j|^2\right) = 4\left(1 + \frac{|\dft{g}_j|^2}{\noiseSD^2}\right), \qquad j+k = n.\]
...
The primary challenge in dealing with statistics of the DFT applied to white noise is that the covariance matrices of $A\dVec$ and $B\dVec$ ($A$ and $B$ representing the real and imaginary parts of the DFT matrix $F$, respectively) are singular. Fortunately, this is not the case for the DCT. \par 
...
\[\dct{\dVec} \sim \mathcal{N}\left(\dct{\dVec},\noiseSD^2 I\right)\]
where $\dVec = \bVec + \noiseVec$. Therefore the DCT-version of Theorem \ref{thm:Mag. squared theorem ext.} is more simple.
\begin{theorem}
Let $\dVec$ be a random $n$-vector with $\dVec \sim \mathcal{N}(\bVec,\noiseSD^2 I)$ and let $\dct{\dVec}$ denote the DCT of $\dVec$. Then for all $0 \leq j \leq n-1$,
\[\frac{1}{\noiseSD^2}\left|\dct{\dVec}_j\right|^2 \sim \NCchi\left(1,\frac{1}{2\noiseSD^2}\left(\dct{g}_j\right)^2\right)\]
where $\NCchi^2(k,\lambda)$ denotes the noncentral chi-squared distribution with $k$ degrees of freedom and noncentrality parameter $\lambda$.
\label{thm:DCT Dist}
\end{theorem}
\begin{proof}
