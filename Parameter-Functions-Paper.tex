...
If the weight vectors $\{\wVec^{(p)}\}_{p=1}^{P}$ satisfy \eqref{eq:Non-overlapping window condition}, then $\|\rWinBig(\regparamVecBig)\|_2^2$ can be written as
\[\left\|\rWinBig(\regparamVecBig)\right\|_2^2 = \sum_{p=1}^{P} \left\|\rVec^{(p)}(\regparam)\right\|_2^2\]
where
