...
\[\lim_{\|\regparamVec\|_2 \rightarrow \infty} \left\|\rWin(\regparamVec)\right\|_2^2 \leq \left\|\dVec\right\|_2^2, \quad \regparamVec \in \mathbb{R}_+^P.\]
\end{lemma}
\begin{proof}
...
From \eqref{eq:Redefinition of Filter}, we know that $0 \leq w_{j+k}^{(p)} \leq 1$ for all $p = 1,\ldots,P$ and $0 \leq \mfilt_{j+k}(\regparam^{(p)}) \leq 1$ for all $\regparam^{(p)} \in \mathbb{R}_+$.
...
\begin{figure}[ht]
\includegraphics[scale=0.32]{Figures/ValidationSet2}
...
\begin{figure}[ht]
\includegraphics[scale=0.32]{Figures/SpectrumDecay}
\caption{}
