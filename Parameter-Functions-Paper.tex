...
\[\left[\Phi(\regparam)\right]_{k,k} = \filt(\regparam,\gamma_k), \quad k = 1,\ldots,n.\]
...
For $p = 1,\ldots,P$, let $\wVec^{(p)} \in \mathbb{R}^n$ be vectors containing the non-negative weights that satisfies
\begin{equation}
\label{eq:Weights}
\sum_{p=1}^{P} \wVec_j^{(p)} = 1, \quad j = 1,\ldots,n.
\end{equation}
Defining windows $W^{(p)} = \diag\left(\wVec^{(p)}\right)$ for $p = 1,\ldots,P$, property \eqref{eq:Weights} implies that $\sum_{p=1}^P W^{(p)} = I$.  A regularization parameter $\regparam^{(p)}$ can be selected for each window $W^{(p)}$ so that a windowed regularized solution can be constructed as
\begin{equation}
...
\label{eq:Windowed GSVD Solution Sum}
    \xWin(\regparamVec) = \sum_{k=1}^n \left(\sum_{p=1}^P \filt_k\left(\regparam^{(p)},\gamma_k\right)\wVec^{(p)}_{k}\right)  \frac{\dft{\dVec}_{k}}{\delta_k} Y_{\cdot,k}.
\end{equation}
...
Given $\noiseVec^{(r)} \sim \mathcal{N}(\zeroVec^{(r)},\Sigma^{(r)})$ for $r = 1,\ldots,R$, assume that $\Sigma^{(r)} = \noiseSD_r^2 I$.
\end{assumption}
\noindent In particular, Assumptions \ref{Assumption_System} and \ref{Assumption_Matrices} imply that $n_r = n$ for $r = 1,\ldots,R$.
...
\E\left(\frac{1}{n}\left\|\pVec(\regparam)\right\|_2^2\right) = \frac{1}{n}\left\|\left(\A - I\right)A\xVec\right\|_2^2 + \frac{1}{n}\trace\left(\A\Sigma\trans{\A}\right).
\end{equation}
...
\begin{align*}
    &\trace\left(\trans{\left(\A - I\right)}\Sigma\left(\A - I\right) \right) \\
    &= \trace\left(\trans{\A}\Sigma\A\right) - \trace\left(\trans{\A}\Sigma\right) - \trace\left(\Sigma\A\right) + \trace\left(\Sigma\right).
\end{align*}
...
\[\trace\left(\trans{\left(\A - I\right)}\Sigma\left(\A - I\right) \right) = \trace\left(\A\Sigma\trans{\A}\right) - 2\trace\left(\Sigma\A\right) + \trace\left(\Sigma\right),\]
...
\widetilde{\U}(\regparam) = \frac{1}{R} \sum_{r=1}^R \U^{(r)}(\regparam),
\end{equation}
where
\begin{equation}
\label{eq:Individual UPRE}
\U^{(r)}(\regparam) = \frac{1}{n}\left\|\rVec^{(r)}(\regparam)\right\|_2^2 + \frac{2}{n} \trace\left(\Sigma^{(r)} A^{(r)}(\regparam)\right) - \frac{1}{n} \trace\left(\Sigma^{(r)}\right).
\end{equation}
with
\begin{equation}
...
\begin{equation}
\label{eq:UPRE 3}
\widetilde{\U}(\regparam) = \frac{1}{N}\left\|\rBig(\regparam)\right\|_2^2 + \frac{2}{N}\trace\left(\widetilde{\Sigma}\ABig(\regparam)\right) - \frac{1}{N}\trace\left(\widetilde{\Sigma}\right).
\end{equation}
...
\begin{equation}
\label{eq:UPRE 4}
\widetilde{\U}(\regparam) = \frac{1}{N}\left\|\rBig(\regparam)\right\|_2^2 + \frac{2}{N} \sum_{r=1}^R \trace\left(\Sigma^{(r)}A^{(r)}(\regparam)\right) - \frac{1}{N} \sum_{r=1}^R \trace\left(\Sigma^{(r)}\right).
\end{equation}
The squared 2-norm of $\rBig(\regparam)$ can also be expressed as a sum so that
\begin{align*}
\widetilde{\U}(\regparam) &= \frac{1}{N} \sum_{r=1}^R \left\|\rVec^{(r)}(\regparam)\right\|_2^2 + \frac{2}{N} \sum_{r=1}^R \trace\left(\Sigma^{(r)} A^{(r)}(\regparam)\right) - \frac{1}{N} \sum_{r=1}^R \trace\left(\Sigma^{(r)}\right) \\
&= \frac{1}{R} \sum_{r=1}^R \left(\frac{1}{n}\left\|\rVec^{(r)}(\regparam)\right\|_2^2 + \frac{2}{n} \trace\left(\Sigma^{(r)} A^{(r)}(\regparam)\right) - \frac{1}{n} \trace\left(\Sigma^{(r)}\right)\right) \\
&= \frac{1}{R} \sum_{r=1}^R \U^{(r)}(\regparam),
\end{align*}
where
\[\U^{(r)}(\regparam) = \frac{1}{n}\left\|\rVec^{(r)}(\regparam)\right\|_2^2 + \frac{2}{n} \trace\left(\Sigma^{(r)} A^{(r)}(\regparam)\right) - \frac{1}{n} \trace\left(\Sigma^{(r)}\right), \quad r = 1,\ldots,R.\]
\end{proof}
\noindent In other words, $\widetilde{\U}(\regparam)$ is the average of $\{\U^{(r)}(\regparam)\}_{r=1}^R$. The UPRE method for multiple data sets is to let $\widetilde{\regparam}_{\textrm{UPRE}} = \argmin_{\regparam > 0} \widetilde{\U}(\regparam)$. As a final remark, \eqref{eq:Individual UPRE} is equivalent to \eqref{eq:UPRE} under Assumption \ref{Assumption_Noise}. \par
...
\begin{align}
\frac{1}{n}\|\rVec^{(r)}(\regparam)\|_2^2 &= \frac{1}{n}\left\|U\left(\Delta^{(r)}\left[\trans{(\Delta^{(r)})}\Delta^{(r)} + \regparam^2\trans{(\Lambda^{(r)})}\Lambda^{(r)}\right]^{-1}\trans{(\Delta^{(r)})} - I\right)\trans{U}\dVec^{(r)}\right\|_2^2 \nonumber \\
&= \frac{1}{n}\left\|\left(\Delta^{(r)}\left[\trans{(\Delta^{(r)})}\Delta^{(r)} + \regparam^2\trans{(\Lambda^{(r)})}\Lambda^{(r)}\right]^{-1}\trans{(\Delta^{(r)})} - I\right)\dft{\dVec}^{(r)}\right\|_2^2 \nonumber \\
&= \frac{1}{n}\sum_{j=1}^{n} \left(\mfilt^{(r)}_j\right)^2\left(\dft{\dVec}_j^{(r)}\right)^2,
...
\U^{(r)}(\regparam) = \frac{1}{n} \sum_{j=1}^{n} \left(\mfilt^{(r)}_j\right)^2\left(\dft{\dVec}_j^{(r)}\right)^2 + \frac{2\noiseSD^2_r}{n} \sum_{j=1}^{n} \filt^{(r)}_j - \noiseSD^2_r, \quad r = 1,\ldots,R.
\end{equation}
...
\overline{U}(\regparam) = \frac{1}{n}\sum_{j=1}^{n} \left(\mfilt_j\right)^2\left(\dft{\aVec}_j\right)^2 + \frac{2}{n} \left(\frac{1}{R^2} \sum_{r=1}^R \noiseSD_r^2\right) \left(\sum_{j=1}^{n} \filt_j\right) - \frac{1}{R^2} \sum_{r=1}^R\noiseSD_r^2.
\end{equation}
Then
\begin{equation}
\label{eq:UPRE Bound}
\widetilde{U}(\regparam) = \frac{1}{R} \sum_{r=1}^R \U^{(r)}(\regparam) \leq R \overline{U}(\regparam), \qquad \regparam \geq 0.
\end{equation}
\end{proposition}
\begin{proof}
...
\begin{align*}
\frac{1}{R} \sum_{r=1}^R \U^{(r)}(\regparam) &= \frac{1}{R} \sum_{r=1}^R \left[\frac{1}{n}\sum_{j=1}^{n} \left(\mfilt_j\right)^2\left(\dft{\dVec}_j^{(r)}\right)^2 + \frac{2\noiseSD_r^2}{n} \sum_{j=1}^{n} \filt_j - \noiseSD_r^2\right] \\
&= \frac{1}{R} \sum_{r=1}^R \left(\frac{1}{n}\sum_{j=1}^{n} \left(\mfilt_j\right)^2\left(\dft{\dVec}_j^{(r)}\right)^2\right) + \frac{1}{R} \sum_{r=1}^R \left(\frac{2\noiseSD_r^2}{n} \sum_{j=1}^{n} \filt_j\right) - \frac{1}{R} \sum_{r=1}^R\noiseSD_r^2 \\
&= \frac{1}{n}\sum_{j=1}^{n} \left(\mfilt_j\right)^2\left(\frac{1}{R} \sum_{r=1}^R \left(\dft{\dVec}_j^{(r)}\right)^2\right) + \frac{2}{n} \left(\frac{1}{R} \sum_{r=1}^R \noiseSD_r^2\right) \left(\sum_{j=1}^{n} \filt_j\right) - \frac{1}{R} \sum_{r=1}^R\noiseSD_r^2.
\end{align*}
Multiplying through by $R/R$ then yields
\begin{align}
\label{eq:Non-average UPRE}
&\frac{1}{R} \sum_{r=1}^R \U^{(r)}(\regparam) \nonumber \\
&= R\left[\frac{1}{n}\sum_{j=1}^{n} \left(\mfilt_j\right)^2\left(\frac{1}{R^2} \sum_{r=1}^R \left(\dft{\dVec}_j^{(r)}\right)^2\right) + \frac{2}{n} \left(\frac{1}{R^2} \sum_{r=1}^R \noiseSD_r^2\right) \left(\sum_{j=1}^{n} \filt_j\right) - \frac{1}{R^2} \sum_{r=1}^R\noiseSD_r^2\right].
\end{align}
...
\[\overline{U}(\regparam) = \frac{1}{n}\sum_{j=1}^{n} \left(\mfilt_j\right)^2\left(\dft{\aVec}_j\right)^2 + \frac{2}{n} \left(\frac{1}{R^2} \sum_{r=1}^R \noiseSD_r^2\right) \left(\sum_{j=1}^{n} \filt_j\right) - \frac{1}{R^2} \sum_{r=1}^R\noiseSD_r^2,\]
i.e. $\overline{U}(\regparam)$ is the UPRE function as applied to the average $\aVec$ of the data $\{\dVec^{(r)}\}_{r=1}^R$, we have that
\[\widetilde{U}(\regparam) = \frac{1}{R} \sum_{r=1}^R \U^{(r)}(\regparam) \leq R \overline{U}(\regparam), \qquad \regparam \geq 0\]
\end{proof}
...
We now derive a multiparameter version of the UPRE function for a single data set $\dVec = A\xVec + \noiseVec$ with $A \in \mathbb{R}^{m \times n}$ and $\noiseVec \sim \mathcal{N}(\zeroVec,\noiseSD^2 I)$. For $p = 1,\ldots,P$, let $\wVec^{(p)}$ be weight vectors satisfying \eqref{eq:Weights} and let $W^{(p)} = \diag(\wVec^{(p)})$. For the regularized solution \eqref{eq:TikSolWindow}, the regularization matrix $R_\text{win}(\regparamVec)$ is
\begin{equation}
...
\begin{equation}
    \rVec_\text{win}(\regparamVec) = U\left(S\left[\sum_{p=1}^P V\inv{\left(\trans{S}S + \regparam_{p}^2\right)}W^{(p)}\right]\trans{S}-I\right)\dft{\dVec}.
\end{equation}
...
\label{eq:Multiparameter UPRE 2}
    U_\text{win}(\regparamVec) = \frac{1}{n}\sum_{j=1}^{n} \left(\sum_{p=1}^{P} w_j^{(p)} \mfilt\left(\regparam^{(p)},\singular_j\right) \right)^2 \left(\dft{d}_j\right)^2 + \frac{2\noiseSD^2}{n}\sum_{j=1}^{n} \left(\sum_{p=1}^{P} w_j^{(p)} \filt\left(\regparam^{(p)},\singular_j\right) \right) - \noiseSD^2.
\end{equation}
...
\begin{proposition}
Using the constructions in Table \ref{tab:System assumptions} under Assumption \ref{Assumption_System} with $n_\ell = n$ for all $r = 1,\ldots,R$, the MDP function $\widetilde{\D}(\regparam)$ for the data sets $\{\dVec^{(r)}\}_{r=1}^R$ is
\begin{equation}
\label{eq:Averaged MDP}
\widetilde{\D}(\regparam) = \frac{1}{R} \sum_{r=1}^R \D^{(r)}(\regparam),
\end{equation}
where
\begin{equation}
\label{eq:Individual MDP}
\D^{(r)}(\regparam) = \D^{(r)}(\regparam) = \frac{1}{n}\|\rVec^{(r)}(\regparam)\|_2^2 - \frac{1}{n}\trace\left(\Sigma^{(r)}\right), \quad r = 1,\ldots,R
\end{equation}
...
\begin{equation}
\label{eq:Big MDP}
\widetilde{\D}(\regparam) = \frac{1}{N}\|\rBig(\regparam)\|_2^2 - \frac{1}{N}\trace\left(\widetilde{\Sigma}\right).
\end{equation}
...
\begin{align*}
\widetilde{\D}(\regparam) &= \frac{1}{N}\sum_{r=1}^R \|\rVec^{(r)}(\regparam)\|_2^2 - \frac{1}{N}\sum_{r=1}^R \trace\left(\Sigma^{(r)}\right) \nonumber \\
&= \frac{1}{R}\sum_{r=1}^R \frac{1}{n}\|\rVec^{(r)}(\regparam)\|_2^2 - \frac{1}{R}\sum_{r=1}^R \frac{1}{n}\trace\left(\Sigma^{(r)}\right) \nonumber \\
...
\overline{\D}(\regparam) = \frac{1}{n}\sum_{j=1}^{n} \left(\mfilt_j\right)^2 |\dft{\aVec}_j|^2 - \frac{1}{R^2}\sum_{r=1}^R \noiseSD_{r}^2
\end{equation}
Then
\begin{equation}
\label{eq:MDP Bound}
\widetilde{\D}(\regparam) = \frac{1}{R} \sum_{r=1}^R \D^{(r)}(\regparam) \leq R \overline{\D}(\regparam), \qquad \regparam \geq 0.
\end{equation}
\end{proposition}
\begin{proof}
Under these assumptions, for the MDP method we have $\widetilde{\D}(\regparam)$ is equal to
\begin{align*}
    \frac{1}{R} \sum_{r=1}^R \D^{(r)}(\regparam) &= \frac{1}{R}\sum_{r=1}^R \left[\frac{1}{n}\|\rVec^{(r)}(\regparam)\|_2^2 - \noiseSD_{r}^2\right] \\
    &= R\left[\frac{1}{n}\sum_{j=1}^{n} \left(\mfilt_j\right)^2\left(\frac{1}{R^2} \sum_{r=1}^R |\dft{\dVec}_j^{(r)}|^2\right) - \frac{1}{R^2}\sum_{r=1}^R \noiseSD_{r}^2\right]
\end{align*}
...
\begin{equation}
\label{eq:GCV}
\G(\regparam) = \frac{\frac{1}{n}\|\rReg\|_2^2}{\left[\frac{1}{n}\trace(I-\A)\right]^2} = \frac{\frac{1}{n}\|\rReg\|_2^2}{\left[1 - \frac{1}{n}\trace(\A)\right]^2}.
\end{equation}
The GCV method determines $\regparam$ such that $\regparam_\GCV = \argmin_{\regparam > 0} \G(\regparam)$.

\begin{proposition}
Using the constructions in Table \ref{tab:System assumptions} under Assumption \ref{Assumption_System} with $n_r = n$ for all $r = 1,\ldots,R$, the GCV function $\widetilde{\D}(\regparam)$ for the data sets $\{\dVec^{(r)}\}_{r=1}^R$ is
\begin{equation}
\label{eq:GCV Big 2}
\widetilde{\G}(\regparam) = \frac{\frac{1}{N}\sum_{r=1}^R \|\rVec^{(r)}(\regparam)\|_2^2}{\left[1 - \frac{1}{N}\sum_{r=1}^R \trace\left(A^{(r)}(\regparam)\right)\right]^2}.
\end{equation}
\end{proposition}
\begin{proof}
...
\begin{equation}
\label{eq:GCV Big}
\widetilde{\G}(\regparam) = \frac{\frac{1}{N}\|\rBig(\regparam)\|_2^2}{\left[1 - \frac{1}{N}\trace\left(\ABig(\regparam)\right)\right]^2}.
\end{equation}
...
\G^{(r)}(\regparam) = \frac{\frac{1}{n}\|\rVec^{(r)}(\regparam)\|_2^2}{\left[1 - \frac{1}{n}\trace\left(\A^{(r)}\right)\right]^2}, \quad r = 1,\ldots,R.
\end{equation}
...
\widetilde{\G}(\regparam) = \frac{\frac{1}{N}\sum_{r=1}^R \|\rVec^{(r)}(\regparam)\|_2^2}{\left[1 - \frac{1}{N}\sum_{r=1}^R \trace\left(\A\right)\right]^2} = \frac{1}{R}\frac{\sum_{r=1}^R \frac{1}{n} \|\rVec^{(r)}(\regparam)\|_2^2}{\left[1 - \frac{1}{n} \trace\left(\A\right)\right]^2} = \frac{1}{R}\sum_{r=1}^R \G^{(r)}(\regparam).
\end{equation}
...
\begin{equation}
\label{eq:GCV Bound}
\widetilde{\G}(\regparam) = \frac{1}{R} \sum_{r=1}^R \G^{(r)}(\regparam) \leq R \overline{\G}(\regparam), \qquad \regparam \geq 0.
\end{equation}
\end{proposition}

\begin{proof}
With these assumptions,
\[\widetilde{\G}(\regparam) = \frac{1}{R} \sum_{r=1}^R \G^{(r)}(\regparam) = \frac{1}{R}\sum_{r=1}^R \left[\frac{\frac{1}{n}\|\rVec^{(r)}(\regparam)\|_2^2}{\left[1 - \frac{1}{n}\trace\left(\A\right)\right]^2}\right]  = R\frac{\frac{1}{n}\sum_{j=1}^{n} \left(\mfilt_j\right)^2\left(\frac{1}{R^2} \sum_{r=1}^R |\dft{\dVec}_j^{(r)}|^2\right)}{\left[1 - \frac{1}{n}\trace\left(\A\right)\right]^2},\]
...
F(\regparam) = \frac{1}{R}\sum_{r=1}^R \left\|\xVec^{(r)}(\regparam) - \xVec^{(r)}\right\|_2^2,
\end{equation} 
...
B^{(r)}(\regparam) = \left\|\xVec^{(r)}(\regparam) - \xVec^{(r)}\right\|_2^2, \quad r = 1,\ldots,R.
\end{equation}
...
\[F(\regparam) = \frac{1}{R}\sum_{r=1}^R B^{(r)}(\regparam).\]
