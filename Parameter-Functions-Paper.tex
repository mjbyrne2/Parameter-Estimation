...
\xWin(\regparamVec) = \sum_{\ell=1}^p V\inv{\left[\trans{S}S + (\regparam^{(\ell)})^2\right]}W^{(\ell)}\trans{S}\dft{\dVec}.
\end{equation}
...
\label{eq:Windowed GSVD Solution}
    \xWin(\regparamVec) = \sum_{\ell=1}^p Y\inv{\left[\trans{\Delta}\Delta + \left(\regparam^{(\ell)}\right)^2 \trans{\Lambda}\Lambda\right]}W^{(\ell)}\trans{\Delta}\dft{\dVec}
\end{equation}
...
\label{eq:Windowed GSVD Solution Sum}
    \xWin(\regparamVec) = \sum_{k=1}^n \left(\sum_{\ell=1}^p \filt_k\left(\regparam^{(\ell)},\gamma_k\right)\wVec^{(\ell)}_{k}\right)  \frac{\dft{\dVec}_{k}}{\delta_k} Y_{\cdot,k}.
\end{equation}
...
\end{equation}
A parameter selection method is generally utilized to select the regularization parameter for each system. Instead, let $\dBig$ be the vector formed by vertically concatenating the data sets $\{\dVec^{(\ell)}\}_{\ell=1}^R$ and define the functional
\begin{equation}
...
\end{equation}
where $\widetilde{\regparam}$ is a single regularization parameter. This construction implies that $\xBig$, $\bBig$, $\widetilde{\noiseVec}$ are vertical concatenations of vectors in $\{\bVec^{(\ell)}\}_{\ell=1}^R$ and $\{\noiseVec^{(\ell)}\}_{\ell=1}^R$, respectively. The matrices $\ABig$ and $\LBig$ are block diagonal matrices with diagonal blocks $\{A^{(\ell)}\}_{\ell=1}^R$ and $\{L^{(\ell)}\}_{\ell=1}^R$, respectively. By the definition of the 2-norm and the construction of \eqref{eq:Big functional}, we can also write
\begin{equation}
\label{eq:Big functional 2}
\widetilde{T}\left(\xBig;\widetilde{\regparam}\right) = \sum_{\ell = 1}^R \left(\|A^{(\ell)}\xVec^{(\ell)} - \dVec^{(\ell)}\|_2^2 + \widetilde{\regparam}^2\|L^{(\ell)} \xVec^{(\ell)}\|_2^2\right) = \sum_{\ell=1}^R T^{(\ell)}\left(\xVec^{(\ell)};\widetilde{\regparam}\right).
\end{equation}
...
To obtain an explicit representation of $\xBig(\regparamBig)$, the GSVD can be utilized. A GSVD of each pair $(A^{(\ell)},L^{(\ell)})$ can obtained for $\ell = 1,\ldots,R$ so that
\begin{equation}
    A^{(\ell)} = U^{(\ell)}\Delta^{(\ell)}\trans{\left(X^{(\ell)}\right)}, \quad L^{(\ell)} = V^{(\ell)}\Lambda^{(\ell)}\trans{\left(X^{(\ell)}\right)}.
\end{equation}
...
\begin{table}[ht!]
  \begin{center}
    \caption{Matrices constructed from the GSVD's of $(A^{(\ell)},L^{(\ell)})$ can obtained for $\ell = 1,\ldots,R$.}
...
\end{equation}
where $\widetilde{Y}$ is the inverse of $\widetilde{X}$. \par
...
The ultimate situation we consider is the case where multiparameter regularization is applied to multiple data sets. Letting $\regparamVec^{(q)}$ be the regularization parameters used for multiparameter regularization applied to the $q$th system described by \eqref{eq:Big vectors}, we can independently construct regularized solutions
\[\xWin^{(q)}\left(\regparamVec^{(q)}\right) = \sum_{\ell=1}^p Y^{(\ell)}\inv{\left[\trans{\Delta}\Delta^{(q)} + \left(\regparam^{(\ell)}\right)^2 \trans{\Lambda}\Lambda^{(q)}\right]}W^{(\ell)}\trans{\Delta}\dft{\dVec}^{(q)}\]



The primary assumption we make moving forward is that the windows $W^{(\ell)}$ for each system are the same, i.e $W^{(\ell)}_q = W^{(\ell)}$ for all $q = 1,\ldots,R$. This implies that $\regparamVec^{(\ell)}$ are all vectors of length $p$.
...
The ultimate situation is multiparameter regularization as applied to multiple data. For this, we combine the notations of Sections \ref{sec:Multiparameter} and \ref{sec:Multiple data sets} to construct a regularized residual $\rWinBig(\regparamVecBig)$ defined as
\begin{equation}
...
\begin{equation}
\label{eq:GCV Big 2}
\widetilde{\G}(\regparam) = \frac{\frac{1}{N}\sum_{\ell=1}^R \|\rReg^{(\ell)}\|_2^2}{\left[1 - \frac{1}{N}\sum_{\ell=1}^R \trace\left(\A^{(\ell)}\right)\right]^2}.
\end{equation}
\end{proposition}
\begin{proof}
...
\G^{(\ell)}(\regparam) = \frac{\frac{1}{n}\|\rReg^{(\ell)}\|_2^2}{\left[1 - \frac{1}{n}\trace\left(\A^{(\ell)}\right)\right]^2}, \quad \ell = 1,\ldots,R.
\end{equation}
...
\widetilde{\G}(\regparam) = \frac{\frac{1}{N}\sum_{\ell=1}^R \|\rReg^{(\ell)}\|_2^2}{\left[1 - \frac{1}{N}\sum_{\ell=1}^R \trace\left(\A\right)\right]^2} = \frac{1}{R}\frac{\sum_{\ell=1}^R \frac{1}{n} \|\rReg^{(\ell)}\|_2^2}{\left[1 - \frac{1}{n} \trace\left(\A\right)\right]^2} = \frac{1}{R}\sum_{\ell=1}^R \G^{(\ell)}(\regparam).
\end{equation}
