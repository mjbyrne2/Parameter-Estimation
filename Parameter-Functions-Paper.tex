...
A = US\trans{V}
\end{equation}
where the $m \times m$ matrix $U$ and the $n \times n$ matrix $V$ are orthogonal and $S$ is a $m \times n$ diagonal matrix. The diagonal elements of $S$ are the singular values of $A$, denoted $\singular_k$ and satisfying $\singular_1 \geq \singular_2 \geq \ldots \geq \singular_{n} \geq 0$. The columns of $U$ and $V$ will be denoted $U_{\cdot,k}$ and $V_{\cdot,k}$, respectively, and are known as the left and right singular vectors of $A$. If $A$ is invertible, then $\inv{A} = V\inv{S}\trans{U}$ and the product $\inv{A}\dVec$ could be used as a solution:
\begin{equation}
\label{eq:InvProd}
\inv{A}\dVec = VS^{-1}{\trans{U}}\dVec = \sum_{k=1}^{n} \frac{{\trans{(U_{\cdot,k})}}\dVec}{\singular_k}V_{\cdot,k} = \sum_{k=1}^{n} \frac{\svd{\dVec}_k}{\singular_k}V_{\cdot,k}.
\end{equation}
where $\svd{\dVec} = \trans{U}\dVec$. Even if $A$ is singular, the pseudoinverse $A^\dagger = V{\Sigma^\dagger}\trans{U}$, where $S^\dagger = \diag(1/\singular_1,\ldots,1/\singular_{r})$, can be used to obtain a solution $A^\dagger\dVec = V{S^\dagger}\svd{\dVec}$. The summation representation of $A^\dagger\dVec$ is analogous to \eqref{eq:InvProd} except the upper limit of summation becomes $r$ where $r = \rank(A)$. In either the nonsingular or singular case for $A$, however, the summands in \eqref{eq:InvProd} are numerically unstable for small $\singular_k$ when the terms $|\svd{\dVec}_k|$ do not decay as fast as the $s_k$; this describes the discrete Picard condition \cite{Hansen:98}. For an ill-conditioned matrix $A$, forming \eqref{eq:InvProd} will often result in a meaningless solution. \par
...
\end{equation}
which is a solution to a damped least squares problem \cite{ABT}. The solutions \eqref{eq:TikSol} and \eqref{eq:Damped LS} can be considered a solution to the ordinary least squares problem
\begin{equation}
\label{eq:Ordinary LS}
\min_{\xVec \in \mathbb{R}^n} \left\|
\begin{bmatrix}
A \\
\regparam I
\end{bmatrix}\xVec - 
...
\begin{equation}
\mathbf{y}(\regparam) = \argmin_{\mathbf{y} \in \mathbb{R}^n} \|B\mathbf{y} - \dVec\|_2^2 + \regparam^2\|\mathbf{y}\|_2^2.
...
\end{equation}
Certainly the structure of the $\Delta$ depends upon the dimension of $A$. If $m \geq n$, then $\Delta$ is a diagonal matrix. Let a $k$-diagonal matrix be defined as an $m \times n$ matrix with nonzero entries given by \eqref{eq:k-diagonal} when $m < n$ (which implies $k \neq 0$). In other words, a $k$-diagonal matrix is a rectangular matrix where the only entries that are possibly nonzero are located on the $k$th upper diagonal. With this definition, $\Delta$ is a $k$-diagonal matrix if $m < n$. \par 
...
\end{align}
Certainly $\trans{\Lambda}\Lambda$ is diagonal, but the diagonality of $\trans{\Delta}\Delta$ may not be obvious if $m < n$. The structure of $\trans{\Delta}\Delta$ in such a case can be shown from the following lemma.
...
\end{proof}
\noindent The diagonality of $\trans{\Delta}\Delta$ follows from part (a) of Lemma \ref{lem:k-diagonal}, which is stated as the following corollary.

\begin{corollary}
The $n \times n$ matrix $\trans{\Delta}\Delta$ in \eqref{eq:Normal equation 2} is diagonal.
\end{corollary}
\begin{proof}
The $m \times n$ matrix $\Delta$ is diagonal if $m \geq n$, in which case the diagonality of $\trans{\Delta}\Delta$ is immediate. If $m < n$, then $\Delta$ is a $k$-diagonal matrix with $k = n - m$. Part (a) of Lemma \ref{lem:k-diagonal} can be applied with $D = \Delta$ and $B = I$, which implies that the structure of $\trans{\Delta}\Delta$ is
...
\end{equation}
where $\bm{\delta} = \diag(\trans{\Delta}\Delta)$, $\bm{\lambda} = \diag(\trans{\Lambda}\Lambda)$, and $Y$ is the inverse of $\trans{X}$. Filter functions similar to \eqref{eq:TikFilt} can be introduced to further simplify notation:
\begin{equation}
\label{eq:Filter functions}
\filt_j = \frac{\bm{\delta}_j}{\bm{\delta}_j + \regparam^2 \bm{\lambda}_j}, \quad \mfilt_j = 1 - \filt_j = \frac{\regparam^2\bm{\lambda}_j}{\bm{\delta}_j + \regparam^2 \bm{\lambda}_j}.
\end{equation}
Using $\filt_j$, the solution \eqref{eq:GSVD Solution 1} can be written as
\begin{equation}
\label{eq:GSVD Solution 2}
\xVec(\regparam) = \sum_{k = 1}^{n} \frac{\bm{\delta}_k}{\bm{\delta}_k + \regparam^2\bm{\lambda}_k} \frac{\svd{\dVec}_k}{\sqrt{\bm{\delta}_k}} Y_{\cdot,k} = \sum_{k = 1}^{n} \filt_j \frac{\svd{\dVec}_k}{\sqrt{\bm{\delta}_k}} Y_{\cdot,k}.
\end{equation} 
If terms of the generalized singular values $\bm{\gamma}_k = \sqrt{\bm{\delta}_k/\bm{\lambda}_k}$ for $k = 1,\ldots,n$, the solution \eqref{eq:GSVD Solution 1} could also be written as
\begin{equation}
\label{eq:GSVD Solution 3}
\xVec(\regparam) = \sum_{k = 1}^{n} \frac{\bm{\delta}_k}{\bm{\delta}_k + \regparam^2\bm{\lambda}_k} \frac{\svd{\dVec}_k}{\sqrt{\bm{\delta}_k}} Y_{\cdot,k} = \sum_{k = 1}^{n} \frac{\bm{\gamma}_k^2}{\bm{\gamma}_k^2 + \regparam^2} \frac{\svd{\dVec}_k}{\sqrt{\bm{\delta}_k}} Y_{\cdot,k}.
\end{equation} 
\indent We now consider the situation where we have a collection of data sets $\{\dVec^{(\ell)}\}_{\ell=1}^R$ where 
\begin{equation}
...
%We first derive the UPRE function under the more general condition that $\noiseVec \sim \mathcal{N}(\bm{0},\Sigma)$
...
\A^{(\ell)} = A^{(\ell)}[\trans{(A^{(\ell)})}A^{(\ell)} + \regparam^2\trans{(L^{(\ell)})}L^{(\ell)}]^{-1}\trans{A}, \quad \ell = 1,\ldots,R.
\end{equation} We can then use the following lemma, which is a modification of the Trace Lemma stated in \cite[p.~98]{Vogel:2002}.
\begin{lemma}
...
%\begin{align*}
%\E(\|\fVec + B\noiseVec\|_2^2) &= \E(\langle \fVec + B\noiseVec, \fVec + B\noiseVec\rangle) \\
%&= \E(\|\fVec\|_2^2) + 2\E\left(\langle \fVec, B\noiseVec\rangle\right) + \E(\langle B\noiseVec, B\noiseVec\rangle) \\
%&= \|\fVec\|_2^2 + 2\E(\langle \fVec, B\noiseVec\rangle) + \E(\langle B\noiseVec, B\noiseVec\rangle) \\
%&= \|\fVec\|_2^2 + 2\E(\trans{\fVec}B\noiseVec) + \E(\trans{\noiseVec}\trans{B}B\noiseVec) \\
%&= \|\fVec\|_2^2 + 2 \sum_{j=1}^{n} (\trans{\fVec}B)_j \E(\noise_j) + \sum_{j=1}^{n} \sum_{k=1}^{n} (\trans{B}B)_{j,k} \E(\noise_{j}\noise_{k}).
%\end{align*}
%Since $\noiseVec \sim \mathcal{N}(\bm{\mu},\Sigma)$, $\E(\noise_j) = \mu_j$ and $\E(\noise_j\noise_k) = \Sigma_{j,k}$. Thus
%\[\E(\|\fVec + B\noiseVec\|_2^2) = \|\fVec\|_2^2 + 2\sum_{j=1}^{n} (\trans{\fVec}B)_j \mu_j + \sum_{j=1}^{n}\sum_{k=1}^{n} (\trans{B}B)_{j,k}\Sigma_{j,k}.\]
...
\E\left(\frac{1}{N}\left\|\widetilde{\pVec}(\regparam)\right\|_2^2\right) = \frac{1}{N}\left\|\left(\widetilde{A}_\regparam - I\right)\widetilde{A}\widetilde{\xVec}\right\|_2^2 + \frac{1}{N}\trace\left(\widetilde{A}_\regparam\widetilde{\Sigma}\widetilde{A}_\regparam \right).
\end{equation}
...
\[\E\left(\frac{1}{N}\left\|\widetilde{\rVec}(\regparam)\right\|_2^2\right) = \frac{1}{N}\left\|\left(\widetilde{A}_\regparam - I\right)\widetilde{A}\widetilde{\xVec}\right\|_2^2 + \frac{1}{N}\trace\left(\trans{\left(\widetilde{A}_\regparam - I\right)}\widetilde{\Sigma}\left(\widetilde{A}_\regparam - I\right) \right).\]
...
\[\trace\left(\trans{\left(\widetilde{A}_\regparam - I\right)}\widetilde{\Sigma}\left(\widetilde{A}_\regparam - I\right) \right) = \trace\left(\trans{\widetilde{A}_\regparam}\widetilde{\Sigma}\widetilde{A}_\regparam\right) - 2\trace\left(\widetilde{\Sigma}\widetilde{A}_\regparam\right) + \trace\left( \widetilde{\Sigma}\right),\]
and so \eqref{eq:New Predicitive Risk 1} can be expressed as
\begin{equation}
\label{eq:New Predictive Risk 2}
\E\left(\frac{1}{N}\left\|\widetilde{\pVec}(\regparam)\right\|_2^2\right) = \E\left(\frac{1}{N}\left\|\widetilde{\rVec}(\regparam)\right\|_2^2\right) + \frac{2}{N}\trace\left(\widetilde{\Sigma}\widetilde{A}_\regparam\right) - \frac{1}{N}\trace\left(\widetilde{\Sigma}\right).
\end{equation}
...
\E\left(\frac{1}{N}\left\|\widetilde{\pVec}(\regparam)\right\|_2^2\right) = \E\left(\frac{1}{N}\left\|\widetilde{\rVec}(\regparam)\right\|_2^2\right) + \frac{2}{N} \sum_{\ell=1}^R \trace\left(\Sigma^{(\ell)} A_\regparam^{(\ell)}\right) - \frac{1}{N} \sum_{\ell=1}^R \trace\left(\Sigma^{(\ell)}\right).
\end{equation}
Analogous to the standard UPRE method, we define $\widetilde{\U}(\regparam)$ as
\begin{equation}
\label{eq:UPRE 2}
\widetilde{\U}(\regparam) = \frac{1}{N}\left\|\widetilde{\rVec}(\regparam)\right\|_2^2 + \frac{2}{N} \sum_{\ell=1}^R \trace\left(\Sigma^{(\ell)} A_\regparam^{(\ell)}\right) - \frac{1}{N} \sum_{\ell=1}^R \trace\left(\Sigma^{(\ell)}\right)
\end{equation}
...
\end{equation}
The function \eqref{eq:MDP} stems from the observation that if $\xReg = \xVec$, then $\E(\frac{1}{n}\|\rVec(\regparam)\|_2^2) = E(\frac{1}{n}\|\noiseVec\|_2^2) = \noiseSD^2$. A more relaxed condition is $\frac{1}{n}\|\xReg - \xVec\|_2^2 < \epsilon$, in which case $\E(\frac{1}{n}\|\rVec(\regparam)\|_2^2) < \epsilon + \noiseSD^2$. Unlike the UPRE method, the MDP method is to choose $\regparam_{\MDP}$ as the zero of $\D(\regparam)$.  \par 
Before extending the MDP method to account for multiple data sets, some comments on the behavior of the MDP function \eqref{eq:MDP}. As can be seen differentiating a representation of $\frac{1}{n}\|\rVec(\regparam)\|_2^2$ provided later in Section \ref{sec:Implementation}, $\frac{1}{n}\|\rVec(\regparam)\|_2^2$ is a monotone increasing function of $\regparam$. This does not guarantee, however, the existence of a zero of $\D(\regparam)$. Two situations can result in the absence of a zero: the variance $\noiseSD^2$ being too small, and the term $\frac{1}{n}\|\rVec(\regparam)\|_2^2$ approaching some constant value as $\regparam \rightarrow \infty$.

%In addition, for Tikhonov regularization we have that $\lim_{\regparam^+ \rightarrow 0} \frac{1}{n}\|\rVec(\regparam)\|_2^2 = 0$ which follows from the following lemma.
%\begin{lemma}
%For the Tikhonov influence matrix $A_{\regparam} = A\inv{(\trans{A}A+\regparam^2\trans{L}L)}\trans{A}$, 
%\[\lim_{\regparam^+ \rightarrow 0} A_{\regparam} = I\]
%in the operator norm induced by the 2-norm on $\mathbb{C}^n$.
%\end{lemma}
%\begin{proof}
%Utilizing the GSVD, the matrices $A$ and $L$ can be written as $A = U\Delta\trans{X}$ and $L = V\Lambda\trans{X}$ where $U$ and $V$ are unitary, $X$ is non-singular, $\Lambda$ is diagonal, and $\trans{\Delta}\Delta$ is diagonal (see Section \ref{sec:Introduction}). Substituting the decomposition of $A$ and $L$ into $A_\regparam$ yields
%\begin{align*}
%A_\regparam &= U\Delta\trans{X}\inv{(X\trans{\Delta}\Delta\trans{X}+\regparam^2X\trans{\Lambda}\Lambda\trans{X})}X\trans{\Delta}\trans{U} \\
%&= U\Delta\trans{X}\inv{[X(\trans{\Delta}\Delta+\regparam^2 \trans{\Lambda}\Lambda)\trans{X}]}X\trans{\Delta}\trans{U} \\
%&= U\Delta\inv{(\trans{\Delta}\Delta+\regparam^2 \trans{\Lambda}\Lambda)}\trans{\Delta}\trans{U} \\
%\end{align*}
%Thus
%\begin{align*}
%\lim_{\regparam^+ \rightarrow 0} \left\|A_\regparam - I\right\| &= \lim_{\regparam^+ \rightarrow 0} \|U\Delta\inv{(\trans{\Delta}\Delta+\regparam^2 \trans{\Lambda}\Lambda)}\trans{\Delta}\trans{U} - I\| \\
%&= \lim_{\regparam^+ \rightarrow 0} \|U[\Delta\inv{(\trans{\Delta}\Delta+\regparam^2 \trans{\Lambda}\Lambda)}\trans{\Delta} - I]\trans{U}\| \\
%&= \lim_{\regparam^+ \rightarrow 0} \|\Delta\inv{(\trans{\Delta}\Delta+\regparam^2 \trans{\Lambda}\Lambda)}\trans{\Delta} - I\|
%\end{align*}
%By Lemma \ref{lem:k-diagonal}, the matrix $\Delta\inv{(\trans{\Delta}\Delta+\regparam^2 \trans{\Lambda}\Lambda)}\trans{\Delta}$ is diagonal for all $\regparam$ and thus \\ $\Delta\inv{(\trans{\Delta}\Delta+\regparam^2 \trans{\Lambda}\Lambda)}\trans{\Delta} - I$ is diagonal. Letting $\bm{\delta} = \diag(\trans{\Delta}\Delta)$, $\bm{\lambda} = \diag(\trans{\Lambda}\Lambda)$, and $k = \max \{n-m,0\}$, the diagonal elements of $\Delta\inv{(\trans{\Delta}\Delta+\regparam^2 \trans{\Lambda}\Lambda)}\trans{\Delta} - I$ are
...
%\[\|\Delta\inv{(\trans{\Delta}\Delta+\regparam^2 \trans{\Lambda}\Lambda)}\trans{\Delta} - I\| = \max_{1+k \leq j \leq n} \filt_j\]
...
\indent To evaluate the effectiveness of the adapted parameter selection methods describes in Section \ref{sec:Methods}, three test problem were considered. For all three problems, 15 data vectors were considered as a set of training data and resulting parameters were then applied to validation data set containing another 15 data vectors. A one-dimensional problem serves as a proof of concept and uses MRI data that is built into MATLAB, which is shown in Figure \ref{fig:MRI 1D True}. The other two problems are two-dimensional; one is constructed from the MATLAB MRI data and the other uses image of the surface of Mercury taken by the MESSENGER spacecraft. \par 

\subsection{One-dimensional problem} \label{sec:1D}
The one-dimensional test problem uses the MRI data built into MATLAB, which can be accessed by the command \texttt{load mri.mat}. Five horizontal slices were selected and reformatted as a single image (Figure \ref{fig:MRI 1D True}).  The columns of the image were then multiplied by a symmetric Toeplitz matrix, approximating a Fredholm integral equation of the first kind with a Gaussian kernel of variance 1.  The following $129 \times 128$ penalty matrix $L_1$ was used, which represents an approximation of a first derivative operator with zero boundary conditions:
\[L_1 = \begin{bmatrix}
1 & & & \\
-1 & 1 & &  \\
 & \ddots &  \ddots & \\
 ...
 & & & -1 
\end{bmatrix}.\]
The resulting system matrix in \eqref{eq:TikSol3} has full column rank, and so applying the normal equations in terms of the GSVD results in unique solutions.
...
The second one-dimensional test problem uses the MRI data built into MATLAB, which can be accessed by the command \texttt{load mri.mat}. The built-in data was reformatted as a single image, which is a trimmed concatenation of five consecutive horizontal slices. The columns of this image were then circularly convolved with a Gaussian kernel of variance 0.01. The full sequence of MRI data and the blurred image sequence area shown in Figure \ref{fig:MRI 1D}. Since circular convolution was utilized, the DFT was the primary tool for the MRI test problem. 
...
%\end{bmatrix}
%\end{equation}
%as a matrix discretization of a first derivative operator. The matrix $L$ defined by \eqref{eq:Periodic L} is circulant and thus diagonalized by the unitary DFT matrix $F$. Writing $L = \trans{F}\Lambda{F}$, we have that $\Lambda = \diag(\sqrt{n}\dft{\mathbf{c}})$ where $\mathbf{c}$ is the first column of $L$.
