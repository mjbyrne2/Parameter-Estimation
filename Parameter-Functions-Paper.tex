...
It is interesting to note that the relative errors of solutions obtained for parameters applied to the second validation set (Figure \ref{fig:Validation Set 2}) were less than those of either the training or first validation set. The similarity of error between the training and first validation set is to be expected since both sets consist of images from the same parent set (subimages selected from Figure \ref{fig:MESSENGER True}). 

\begin{table}[ht!]
  \begin{center}
    \caption{Percent relative errors of regularized solutions for $\nu = 200$ and an SNR of 10 with two log cosine windows and a Laplacian penalty matrix.}
...
\caption{Adapted parameters obtained from using two log cosine windows as the number of data sets changes from one to 16. For all three methods, $\regparam$}
