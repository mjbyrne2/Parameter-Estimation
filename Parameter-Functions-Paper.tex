...
\end{equation}
The vectors $\dVec^{(r)}$, $\bVec^{(r)}$, $\noiseVec^{(r)}$, and $\zeroVec^{(r)}$ have length $m_r$, while the vector $\xVec^{(r)}$ has length $n_r$. The system matrices $A^{(r)}$ and covariance matrices $\Sigma^{(r)}$ are thus $m_r \times n_r$ and $m_r \times m_r$, respectively. For given regularization parameters $\regparam^{(r)}$ and penalty matrices $L^{(r)}$ of dimension $q_r \times n_r$, Tikhonov regularization can be performed to produce regularized solutions $\xVec(\regparam^{(r)})$ that minimize the functionals
\begin{equation}
...
Given $\noiseVec^{(r)} \sim \mathcal{N}(\zeroVec^{(r)},\Sigma^{(r)})$ for $r = 1,\ldots,R$, assume that $\zeroVec^{(r)} = \zeroVec \in \mathbb{R}^{\mA}$ and $\Sigma^{(r)} = \noiseSD_r^2 I_{\mA}$.
\end{assumption}
...
\begin{proposition}
Using the constructions in Table \ref{tab:System assumptions} under Assumption \ref{Assumption_System} with $m_r = m$ for all $r = 1,\ldots,R$, the UPRE function $\UBig(\regparam)$ for the data sets $\{\dVec^{(r)}\}_{r=1}^R$ is
\begin{equation}
...
\begin{proposition}
Using the constructions in Table \ref{tab:System assumptions} under Assumption \ref{Assumption_System} with $m_r = m$ for all $r = 1,\ldots,R$, the MDP function $\DBig(\regparam)$ for the data sets $\{\dVec^{(r)}\}_{r=1}^R$ is
\begin{equation}
...
\begin{proposition}
Using the constructions in Table \ref{tab:System assumptions} under Assumption \ref{Assumption_System} with $m_r = m$ for all $r = 1,\ldots,R$, the GCV function $\DBig(\regparam)$ for the data sets $\{\dVec^{(r)}\}_{r=1}^R$ is
\begin{equation}
\label{eq:GCV Big 2}
...
\end{equation}
However, if we assume that $A^{(r)} = A$ and $L^{(r)} = L$ for each $r = 1,\ldots,R$, then $A^{(r)}(\regparam) = \A$ for each $r = 1,\ldots,R$ as well. In such case, \eqref{eq:GCV Big 2} can be expressed as
\begin{equation}
...
\begin{align}
    ...
    &= R\frac{\frac{1}{\mA}\sum_{j=1}^{n} \left(\mfilt_j\right)^2\left(\frac{1}{R^2} \sum_{r=1}^R \left|\dft{\dVec}_j^{(r)}\right|^2\right)}{\left[1 - \frac{1}{\mA}\trace\left(\A\right)\right]^2},
\end{align}
