...
With the compact SVD, the pseudoinverse of $A$ can be expressed as 
\begin{equation}
\label{eq:Pseudoinverse}
    \pinv{A} = V_r\inv{S}_r\trans{U}_r.
\end{equation} 
Representation \eqref{eq:Pseudoinverse} is convenient for the purpose of proving some basic properties of the pseudoinverse presented in Proposition \ref{prop:Pseudoinverse properties}.

\begin{proposition}
\label{prop:Pseudoinverse properties}
Given $A \in \mathbb{R}^{m \times n}$, let $\pinv{A} \in \mathbb{R}^{n \times m}$ denote the pseudoinverse of $A$. Then
\end{proposition}
