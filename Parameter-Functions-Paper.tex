...
\BWinBig(\regparamVec) = \frac{1}{R}\left\|\xWinBig(\regparamVec) - \xBig\right\|_2^2 = \frac{1}{R}\sum_{r=1}^R \BWin^{(r)}(\regparamVec),
\end{equation} 
where
\begin{equation}
\label{eq:Individial Best Function}
    \BWin^{(r)}(\regparamVec) = \left\|\xWin^{(r)}(\regparamVec) - \xVec^{(r)}\right\|_2^2.
\end{equation}
Regularization parameters chosen as minimizers of \eqref{eq:Learning function} are ``best" in the sense of minimizing the mean squared errors of the regularized solutions $\xWin^{(r)}(\regparamVec)$; the use of \eqref{eq:Learning function} was considered in \cite{ChungEspanol2017}. It is important to note that determining minimizers of \eqref{eq:Learning function} relies on knowledge of the true solutions $\{\xVec^{(r)}\}_{r=1}^R$. One could also find minimizers $\regparamVec^{(r)}$ of \eqref{eq:Individial Best Function} for each $r = 1,\ldots,R$, which would produce parameters that are ``best" for their own data set. \par
...
\end{figure}
\noindent When the number of training vectors is small (e.g. between 1 and 10 from Figure \ref{fig:Parameters 1D MRI}), the parameters selected by all four methods change significantly. This can be attributed to the fact that all of the methods select a parameter that is either a root or a minimum of an average of functions. For a small amount of training vectors, each additional vector has more influence over the behavior of the adapted function. The parameters stabilize as the number of training vectors reach a certain point; in the problem being considered here, the parameters stabilize by about 40 training sets. \par 
...
\begin{table}[ht!]
  \begin{center}
    ...
    \hline
    2 & 24.81\% & 24.86\% & 24.86\% & 26.93\% & 26.97\% & 26.97\% & 18.53\% & 18.59\% & 18.59\% \\
    \hline
    4 & 24.67\% & 24.52\% & 24.52\% & 27.01\% & 26.80\% & 26.80\% & 18.72\% & 18.36\% & 18.36\% \\
    \hline
    6 & 24.57\% & 24.55\% & 24.55\% & 26.90\% & 26.79\% & 26.79\% & 18.54\% & 18.33\% & 18.33\% \\
    \hline
    8 & 24.55\% & 24.55\% & 24.55\% & 26.85\% & 26.79\% & 26.79\% & 18.46\% & 18.33\% & 18.33\% \\
    \hline
    \end{tabular}
  \end{center}
\end{table}
