...
\[\left\|\rWinAvg(\regparamVec)\right\|_2^2 \leq \frac{1}{R^2}\sum_{r=1}^R \left(\sum_{j=1}^{m} \left(\left[\sum_{p=1}^{P} w_{j+k}^{(p)}\Psi_{j+k}\left(\regparam^{(p)}\right)\right] \hat{d}_j^{(r)}\right)^2\right) = \frac{1}{R}\left\|\rWinBig(\regparamVec)\right\|_2^2\]
