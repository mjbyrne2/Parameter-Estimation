...
Representation \eqref{eq:TikSol} is then obtained by noting that $(\trans{S}S + \regparam^2 I)$ is a diagonal matrix that is guaranteed to be non-singular for $\regparam > 0$. If $A$ has full column rank and $m \geq n$, then $(\trans{S}S + \regparam^2 I)$ is non-singular even when $\regparam = 0$. \par
 ...
 Representation \eqref{eq:Damped LS} follows from selecting $L$ to be $I$, the $n \times n$ identity matrix. Analogous to \eqref{eq:Ordinary LS}, the solution \eqref{eq:TikSol2} can be expressed in block form as
\begin{equation}
...
\begin{table}[ht!]
  \begin{center}
    ...
    \begin{tabular}{cc} 
    $\dVec^{(\ell)} = \bVec^{(\ell)} + \noiseVec^{(\ell)}$, & $\bVec^{(\ell)} = {A^{(\ell)}}\xVec^{(\ell)}$, \\ 
    $\widetilde{\xVec} = \vcat(\xVec^{(1)},\ldots,\xVec^{(R)})$, & $\noiseVec^{(\ell)} \sim \mathcal{N}(\zeroVec^{(\ell)},\Sigma^{(\ell)})$, \\
    $\widetilde{\dVec} = \vcat(\dVec^{(1)},\ldots,\dVec^{(R)})$, & $\widetilde{\Sigma} = \diag(\Sigma^{(1)},\ldots,\Sigma^{(R)})$, \\
    $\widetilde{\noiseVec} = \vcat(\noiseVec^{(1)},\ldots,\noiseVec^{(R)})$, & $\widetilde{A} = \diag(A^{(1)},\ldots,A^{(R)})$. 
    \end{tabular}
  \end{center}
\end{table}
...
The development of the adapted UPRE, MDP, and GCV methods will rely on the assumptions and constructions contained in Table \ref{tab:System assumptions}. In addition, the development of all three adapted methods will use the additional assumption that $\noiseVec^{(\ell)} \sim \mathcal{N}(\zeroVec^{(\ell)},\Sigma^{(\ell)})$ for all $\ell = 1,\ldots,R$, where the length of $\noiseVec^{(\ell)}$ is $n_\ell$. By Lemma \ref{lem:Concatenation of Normal Noise}, this assumption implies that $\widetilde{\noiseVec} \sim \mathcal{N}\left(\widetilde{\zeroVec},\widetilde{\Sigma}\right)$ with $\widetilde{\zeroVec} \in \mathbb{R}^N$ and $N = \sum_{\ell=1}^{R} n_\ell$. 
